...
def transform_data(df, x_col, y_col, lag=0):
    # Apply lag only within each Brand group (using Date order)
    if lag != 0:
        x_series = df.groupby("Brand")[x_col].shift(lag)
    else:
        x_series = df[x_col]
    
    # Store lagged x values in a new column for further processing
    df["_lagged_x_"] = x_series
    
    # Calculate initial correlation and R² between lagged x and y
    ...
    # Search for the best transformation of lagged x using: new_var = 1 / (1 + (gamma / x)^alpha)
    ...
    # Create new column name reflecting the applied lag
    ...
    # Drop rows with NaN in transformed variable or y_col (could result from lag)
...
# File uploader: CSV must contain at least Date, Brand, and one additional column.
...
if uploaded_file is not None:
    try:
        ...
        # Check required columns Date and Brand
        ...
        # From remaining columns, choose independent (X) and dependent (Y) variables
        ...
        # Allow user to specify a lag value (applied by Brand, using Date order)
        ...
        # Allow user to choose which model to display
        ...
        # Add column with predicted y values using the chosen model
        if chosen_model == "linear":
        ...
        # Automatically display the interactive regression plot
        ...
        # For a single value, lag is not applied; only transformation is applied.
